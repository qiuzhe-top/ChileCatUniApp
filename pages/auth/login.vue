	...
	<view class="u-p-50">
		...
		<u-modal title="修改密码" v-model="up_password_show" @confirm="confirm" ref="uModal" :show-cancel-button="true"
			...
			<view class="up_password">
				...
				<u-input v-model="password_new" placeholder="请输入密码" type="password" />
	...
	export default {
		...
		mounted() {
			...
			if (process.env.NODE_ENV === 'development') {
				this.username = '19510101'
				this.password = '19510101'
			}
			//this.isLogin();
		},
		methods: {
			...
			up_password(){
				this.$store.dispatch('user/ChangePassword', {
						...
						password_old: this.$data.password,
						username: this.$data.username,
					}).then(res => {
						this.up_password_show = false;
						this.$data.password = this.$data.password_new
						this.web_login();
